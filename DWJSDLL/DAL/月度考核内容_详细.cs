...
namespace Eastcom.DAL
{
    ...
    public partial class 月度考核内容_详细
    {
        ...
        public bool Exists(int id)
        {
            ...
            strSql.Append("select count(1) from 月度考核内容_详细");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from 月度考核内容_详细 ");
            strSql.Append(" where id=@id");
            SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)
			};
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select id,FK_月度考核内容,考核题目编号,题目编号,用户答案,回答情况,得分,提交时间,题目内容_原,A选项_原,B选项_原,C选项_原,D选项_原,其他选项_原,标准答案_原  ");
            strSql.Append(" FROM 月度考核内容_详细 ");
            if (strWhere.Trim() != "")
            {
        ...
        public DataSet GetList(int Top, string strWhere, string filedOrder)
        {
            ...
            strSql.Append(" id,FK_月度考核内容,考核题目编号,题目编号,用户答案,回答情况,得分,提交时间,题目内容_原,A选项_原,B选项_原,C选项_原,D选项_原,其他选项_原,标准答案_原");
            strSql.Append(" FROM 月度考核内容_详细 ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
        ...
        public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
        {
            ...
            strSql.Append(")AS Row, T.*  from 月度考核内容_详细 T ");
        ...
        public Eastcom.Model.月度考核内容_详细 GetUndoneId(int FK_月度考核)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 id,FK_月度考核内容,考核题目编号,题目编号,用户答案,回答情况,得分,提交时间,题目内容_原,A选项_原,B选项_原,C选项_原,D选项_原,其他选项_原,标准答案_原  from 月度考核内容_详细 ");
            strSql.Append(" where FK_月度考核内容=@FK_月度考核内容  and  ( 用户答案 IS NULL OR  用户答案 ='' or 用户答案 =',,,' or 用户答案 =',,,,' )  ");
            SqlParameter[] parameters = {
					new SqlParameter("@FK_月度考核内容", SqlDbType.Int,4)
			};
            parameters[0].Value = FK_月度考核;
