...
namespace CommonClass.Mail
{
    public class MyEmail
    {
        ...
        /// 邮件配置文件的路径
        /// </summary>
        public static string GetMailXmlPath
        {
            get {
                return HttpContext.Current.Request.MapPath("~/Config/SysConfig.xml");
            }
        }
        /// <summary>
        /// 构造函数
        ...
        public static MailSetting GetMailSetting
        {
            get {
                ...
                doc.Load(MyEmail.GetMailXmlPath);
                MailSetting smtpSetting = new MailSetting();
                if (null != doc)
                {
                    smtpSetting.Server = doc.DocumentElement.SelectSingleNode("Mail/SmtpServer").InnerText;
                    smtpSetting.Authentication = Convert.ToBoolean(doc.DocumentElement.SelectSingleNode("Mail/Authentication").InnerText);
                    smtpSetting.From = doc.DocumentElement.SelectSingleNode("Mail/From").InnerText;
                    smtpSetting.Password = doc.DocumentElement.SelectSingleNode("Mail/Pwd").InnerText;
                    smtpSetting.ShowName = doc.DocumentElement.SelectSingleNode("Mail/ShowName").InnerText;
                    smtpSetting.SmtpPort = Common.GetInt(doc.DocumentElement.SelectSingleNode("Mail/SmtpPort").InnerText);
                    smtpSetting.IsEnable = Common.GetBool(doc.DocumentElement.SelectSingleNode("Mail/Enable").InnerText);
                }
                return smtpSetting;
            }
        }
        ...
        /// 更新邮件配置
        /// </summary>
        public static void UpdateMailConfig(MailSetting m)
        {
            if (null == m)
            {
                return;
            ...
            if (null != doc)
            {
                doc.DocumentElement.SelectSingleNode("Mail/SmtpServer").InnerText = m.Server;
                doc.DocumentElement.SelectSingleNode("Mail/Authentication").InnerText = Convert.ToString(m.Authentication);
                doc.DocumentElement.SelectSingleNode("Mail/From").InnerText = m.From;
                if (m.Password.Length > 0)
                {
                    doc.DocumentElement.SelectSingleNode("Mail/Pwd").InnerText = m.Password;
                }
                doc.DocumentElement.SelectSingleNode("Mail/ShowName").InnerText = m.ShowName;
                doc.DocumentElement.SelectSingleNode("Mail/SmtpPort").InnerText = Convert.ToString(m.SmtpPort);
                doc.DocumentElement.SelectSingleNode("Mail/Enable").InnerText = Convert.ToString(m.IsEnable);
                doc.Save(MyEmail.GetMailXmlPath);
            }
        }
        #endregion
    }
