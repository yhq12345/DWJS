<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="MyAskHis.aspx.cs" Inherits="DWKS.ASK.MyAskHis" %>
            ...
            <div>
                ...
                <div style="display: inline; float: left;">
                    <input id="save_btn" runat="server" type="button" src="../images/img_query.gif" value="保 存"
                        onclick="funs.Save_Save(this);return false;" visible="false" />&nbsp;
                </div>
                <div style="display: inline; float: left;">
                    <input id="none_done_btn" runat="server" type="button" src="../images/img_query.gif"
                        value="未完成的题" onclick="funs.Save_UnDone(this);return false;"  visible="false" />&nbsp;
                </div>
    ...
    <script type="text/javascript">
        ...
        var funs = {
            Save_Save: function (obj) {
                var that = this;
                $.XCLGoAjax({ data: $(obj).closest("form").serialize() + "&method=ajax&handleType=Save", obj: obj, isLockBtn: false, isChangeBtnVal: false, successFunction: this.sFunction_Save, beforeSendMsg: "跳转中..." });
            },
            Save_Next: function (obj) {
                var that = this;
                $.XCLGoAjax({ data: $(obj).closest("form").serialize() + "&method=ajax&handleType=Save", obj: obj, isLockBtn: false, isChangeBtnVal: false, successFunction: this.sFunction_Next, beforeSendMsg: "跳转中..." });
            },
            Save_Pre: function (obj) {
                var that = this;
                $.XCLGoAjax({ data: $(obj).closest("form").serialize() + "&method=ajax&handleType=Save", obj: obj, isLockBtn: false, isChangeBtnVal: false, successFunction: this.sFunction_Pre, beforeSendMsg: "跳转中..." });
            },
            Save_UnDone: function (obj) {
                var that = this;
                $.XCLGoAjax({ data: $(obj).closest("form").serialize() + "&method=ajax&handleType=Save", obj: obj, isLockBtn: false, isChangeBtnVal: false, successFunction: this.sFunction_UnDone, beforeSendMsg: "跳转中..." });
            },
            ...
            sFunction_Next: function (str) {
                window.location.href = './MyAskHis.aspx?id=<%=nextid %>&YearMonth=<%=YearMonth %>';
            },
            sFunction_Pre: function (str) {
                window.location.href = './MyAskHis.aspx?id=<%=preid %>&YearMonth=<%=YearMonth %>';
            },
            sFunction_UnDone: function (str) {
                ...
                if (c == "0") {
                    ...
                    window.location.href = './MyAskHis.aspx?id=' + c + '&YearMonth=<%=YearMonth %>';
                }
            },
